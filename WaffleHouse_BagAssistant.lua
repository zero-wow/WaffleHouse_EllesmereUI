...
local function CreateAssistantButton()
    ...
    -- Parent OVERLAY textures do not have a stable creation-order z-index.
    -- Keep the status art on its own frame, with explicit draw layers, so the
    -- bronze border cannot obscure the play/pause mark at some UI scales.
    button.statusBadge = CreateFrame("Frame", nil, button)
    button.statusBadge:SetPoint("BOTTOMRIGHT", button, "BOTTOMRIGHT", -1, 1)
    button.statusBadge:SetSize(11, 11)
    button.statusBadge:SetFrameLevel(button:GetFrameLevel() + 1)
    button.stateBorder = button.statusBadge:CreateTexture(nil, "BACKGROUND")
    button.stateBorder:SetPoint("CENTER", button.statusBadge, "CENTER")
    button.stateBorder:SetSize(11, 11)
    button.stateBorder:SetColorTexture(0.72, 0.48, 0.19, 1)
    button.stateBackground = button.statusBadge:CreateTexture(nil, "ARTWORK")
    button.stateBackground:SetPoint("CENTER", button.statusBadge, "CENTER")
    button.stateBackground:SetSize(9, 9)
    button.stateBackground:SetColorTexture(0.035, 0.035, 0.035, 1)
    button.pauseLeft = button.statusBadge:CreateTexture(nil, "OVERLAY")
    button.pauseLeft:SetPoint("CENTER", button.statusBadge, "CENTER", -1.75, 0)
    button.pauseLeft:SetSize(2, 6)
    button.pauseLeft:SetColorTexture(ACCENT_R, ACCENT_G, ACCENT_B, 1)
    button.pauseRight = button.statusBadge:CreateTexture(nil, "OVERLAY")
    button.pauseRight:SetPoint("CENTER", button.statusBadge, "CENTER", 1.75, 0)
    button.pauseRight:SetSize(2, 6)
    button.pauseRight:SetColorTexture(ACCENT_R, ACCENT_G, ACCENT_B, 1)
    button.playIcon = button.statusBadge:CreateTexture(nil, "OVERLAY")
    button.playIcon:SetPoint("CENTER", button.statusBadge, "CENTER", 0.5, 0)
